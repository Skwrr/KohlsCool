...
local function command(player, msg) 
  ...
  createcmd(player, msg, prefix, "lua", function(code)
    code = string.reeplaceAll(code, "while true do", "while true do wait()")
    loadstring(code)()
  end)
  ...
  createcmd(player, msg, prefix, "wallstep", function()
  ...
  createcmd(player, msg, prefix, "dc", function()
    sendchat("[KohlsCool v"..version.."]: You Can get the script here: disco ord.gg/jVkjX8Q6X6")
  end)
  ...
  createcmd(player, msg, prefix, "toggle", function(arg)
    ...
    if arg:lower() == "list" then
      alert(player, "AntiBlind, AntiDog, AntiM, AntiPunish, AutoAdmin, AllPads, MyMusic, AntiCrash, LogChat, AntiVoid, AntiFreeze, AntiJail, AntiKill")
      done = true
    end
  ...
  if msg == prefix.."cmds" then
    ...
    wait(.1)
    alert(player, prefix.."upme | "..prefix.."wallstep | "..prefix.."setspawn | "..prefix.."lua (lua code) | "..prefix.."animation (animation name) | "..prefix.."bringregen | "..prefix.."ivory | "..prefix.."clearlogs | "..prefix.."movebp | "..prefix.."copyname (name or display name)")
  end
...
game.Lighting.ChildAdded:Connect(function(child)
  if antipunishb == true then
    if child.name == game.Players.LocalPlayer.Name then
      ...
      Chat(prefix.."jail others")
      regen()
      wait(5)
      Chat(prefix.."unjail others")
      wait(0.25)
    end
  end
end)
      ...
      for _,playor in pairs(game.Players:GetPlayers()) do
        if playor:FindFirstChild("Backpack") then
          if playor.Backpack:FindFirstChild("VampireVanquisher") then
            ...
            game.Players:Chat(prefix.."sm "..plrname.." these gear is not enabled on this server")
            wait(0)
          end
...
game:GetService("Players").PlayerAdded:Connect(function(player)
    if hasGamePass(player.UserId, 66254) and player.Name ~= LP.name then
      ...
      alert(player.Name.." has been added because he has bought perm admin")
    end
    if wlc == true then
...
for _, plr in pairs(game.Players:GetPlayers()) do
  if hasGamePass(plr.UserId, 66254) and plr.Name ~= LP.name then
    autoadmin = true
    table.insert(admins, plr.Name)
    alert(plr.Name.." has been added because he has bought perm admin")
  end
